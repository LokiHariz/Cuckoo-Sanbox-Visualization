...
class Procmon(Auxiliary):
    ...
    def start(self):
        ...
        self.procmon_pmc = os.path.join(bin_path, "ProcmonConfigurationv2.pmc")
        self.procmon_pml = os.path.join(bin_path, "procmon.pml")
        self.procmon_csv = os.path.join(bin_path, "procmon.csv")
    ...
    def stop(self):
        ...
        # Convert the process monitor log into a CSV file.
        subprocess.check_call([
            self.procmon_exe,
            "/OpenLog", self.procmon_pml,
            "/LoadConfig", self.procmon_pmc,
            "/SaveAs", self.procmon_csv,
            "/SaveApplyFilter",
        ])

        # Upload the CSV file to the host.
        upload_to_host(self.procmon_csv, os.path.join("files", "procmon.csv"))
